                              ...
                              DWORD flags, BOOL system, BOOL redirected, WINE_MODREF **pwm )
{
    ...
    wineforge_apply_pe_patches( wm->ldr.BaseDllName.Buffer, wm->ldr.FullDllName.Buffer,
                                wm->ldr.DllBase, wm->ldr.SizeOfImage, is_builtin );

#if defined(__x86_64__)
    /* CW HACK 22434 */
    if (!is_builtin)
    {
        struct pe_module_loaded_params params = { *module, (char *)*module + map_size };
...
#ifdef _WIN64
    if (!contains_path( libname ) &&
        (status = open_wfdxcompat_runtime_alias( libname, nt_name, pwm, mapping,
